...
python_bin="${CONTACT_SUPPORT_PYTHON:-python3}"

if [[ ! -x "${python_bin}" ]]; then
  echo "contact/support Python is not executable: ${python_bin}" >&2
  exit 1
fi
...
output_dir="${5:-$PWD/runs/deform360-contact-support}"
...
repository_root, bpt_root, deform360_root, official_root = map(Path, sys.argv[1:])
selector_path = (
    repository_root
    / "scripts"
    / "remote"
...
spec = importlib.util.spec_from_file_location(
    "deform360_contact_support_python_selector",
    selector_path,
)
if spec is None or spec.loader is None:
    raise SystemExit("failed to load the conditional reproduction runtime selector")
...
"$python_bin" -m pytest -q \
  "$repository_root/tests/test_deform360_contact_support_diagnostic.py" \
  "$repository_root/tests/test_deform360_replication_case.py" \
  "$repository_root/tests/test_deform360_replication_warp.py" \
  "$repository_root/tests/test_deform360_prefix_kinematics_milestone.py" \
  "$repository_root/tests/test_deform360_source_failure_attribution.py"

result="$output_dir/result.json"
"$python_bin" "$repository_root/scripts/remote/run_deform360_contact_support.py" \
  --repository-root "$repository_root" \
  --data-root "$data_root" \
