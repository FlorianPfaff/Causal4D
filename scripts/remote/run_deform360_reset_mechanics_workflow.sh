...
python_bin="${RESET_MECHANICS_PYTHON:-python3}"

if [[ ! -x "${python_bin}" ]]; then
  echo "reset-mechanics Python is not executable: ${python_bin}" >&2
  exit 1
fi
...
output_dir="${5:-$PWD/runs/deform360-reset-mechanics}"
...
spec = importlib.util.spec_from_file_location(
    "deform360_reset_mechanics_python_selector",
    selector_path,
)
...
"$python_bin" "$repository_root/scripts/remote/run_deform360_reset_mechanics.py" \
  --repository-root "$repository_root" \
  --data-root "$data_root" \
