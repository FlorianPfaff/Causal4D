...
def test_citation_metadata_matches_package_version_repository_and_license() -> None:
    ...
    assert 'License = "https://github.com/IPS-Stuttgart/Causal4D/blob/main/LICENSE"' in (
        pyproject
    )
...
def test_current_documentation_explains_the_license_scope() -> None:
    ...
    assert "historical records" in licensing
    assert "Licensing and scholarly attribution are separate" in licensing
