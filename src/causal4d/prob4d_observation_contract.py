"""Independently validate legacy and joint-gauge Prob4D observation contracts.

Causal4D intentionally validates the producer metadata without importing Prob4D
or Bayesian-PhysTwin. This keeps the causal lineage boundary available in the
lightweight base installation and makes semantic drift fail closed.
"""
    ...
    *,
    ...
    if explicit_v2:
        ...
        _require(
            ...
            "Prob4D stream contract v2 anchor has no calibration_artifact_sha256",
        )
        _require(
            covariance_treatment
            in {FIXED_EXTERNAL_CALIBRATION, PROPAGATED_EXTERNAL_PRIOR},
...
def _validate_factor_semantics(
    ...
    if max_rank is not None:
        _require(
            _require_integer(
                max_rank,
                name="gauge-posterior max_gauge_rank",
            )
            >= factor_rank,
...
def validate_prob4d_causal_observation_metadata(
    ...
    _require(bool(case_id), "Prob4D causal artifact has no case ID")
    window_names = tuple(map(str, descriptor.get("window_names", ())))
    _require(
        bool(window_names),
    ...
    factor_validation = _validate_factor_semantics(
        descriptor,
        arrays,
        metadata,
    )
